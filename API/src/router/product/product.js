...
exports.getAllProducts = async (ctx) => {
    ...
    ctx.body = await productActions.getProducts()
    return ctx
}
...
exports.createProduct = async (ctx) => {
    ...
    let product = await productActions.addProduct(ctx.request.body)
    ctx.body = { message: 'El producto ha sido creado', product }
    return ctx
}

exports.getProductsForCategory = async (ctx) => {
    // Action que busca por categorias, retorna un arreglo de JSONs, en caso de no existir retornara -1
    const result = await productActions.getProductsForCategory(ctx.params.category)
    if (result == -1){
        ctx.status = 404
...
exports.getProductsForCategoryAndOrder = async (ctx) => {
    ...
    const result = await productActions.getProductsForCategoryAndOrder(ctx.params.category,ctx.params.ord)
    if (result == -1){
        ctx.status  = 404
...
exports.updateProduct = async (ctx) => {
    ...
    let products_update = await productActions.updateProduct(ctx.params.id,ctx.request.body)
    if (products_update.length == 0 ){
        ctx.status = 404
        ctx.body = {message: `El producto con id ${ctx.params.id} no existe`}
    }else{
        let product = products_update
...
exports.removeProduct = async (ctx) => {
    ...
    let product_delete = await productActions.removeProduct(ctx.params.id)
    if (product_delete == -1) {
        ctx.status = 404
        ctx.body = {message: `El producto con id ${ctx.params.id} no existe`}
    }else{
